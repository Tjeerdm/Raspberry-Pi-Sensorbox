 ...
 *      unsigned char length, unsigned char *data) (true == ERROR)
 ...
 * min(int a, int b)
 */
static bool i2c_write(unsigned char slave_addr, unsigned char reg_addr,
                      unsigned char length, unsigned char const *data)
{
    char b[1024];
    b[0] = (char)reg_addr;
    ...
    return bcm2835_i2c_write(b, length+1) ? true : false;
}
                     ...
                     unsigned char length, unsigned char *data)
{
    ...
    char cmd = (char)reg_addr;
    bcm2835_i2c_write(&cmd, 1);
    return bcm2835_i2c_read((char*)data, length) ? true : false;
}
...
int mpu_init(struct int_param_s *int_param)
{
    ...
    if (rev) {
        ...
        if (!rev) {
#ifdef MPU_DEBUG
            printf("Product ID read as 0 indicates device is either incompatible or an MPU3050\n");
#endif
            return -6;
        } else if (rev == 4) {
#ifdef MPU_DEBUG
...
#ifdef AK89xx_SECONDARY
#ifdef MPU_DEBUG
    printf("Setting up compass\n");
#endif
    errCode = setup_compass();
    if (errCode != 0) {
#ifdef MPU_DEBUG
       printf("Setup compass failed: %d\n", errCode);
#endif
    }
...
#ifdef MPU_DEBUG
            printf("fimrware write failed: %d\n", errCode);
#endif
            return -3;
        }
...
#ifdef MPU_DEBUG
            printf("Firmware compare failed addr %d\n", ii);
            for (int i = 0; i < 10; i++) {
              printf("%d ", progBuffer[i]); 
            }
            printf("\n");
            for (int i = 0; i < 10; i++) {
              printf("%d ", cur[i]); 
